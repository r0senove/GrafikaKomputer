...
void Demo::ProcessInput(GLFWwindow *window) {
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
		MoveCamera(CAMERA_SPEED); //gerak kamera maju
	}
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
		MoveCamera(-CAMERA_SPEED); //gerak kamera mundur
	}

	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
		StrafeCamera(-CAMERA_SPEED); //gerak kamera ke kiri
	}

	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
		StrafeCamera(CAMERA_SPEED); //gerak kamera ke kanan
	}
...
int main(int argc, char** argv) {
	...
	app.Start("Tugas2: Manipulasi Kamera & Gerakkan Objek 3D", 800, 600, false, false);
}